...
companies_number_by_sector <- nyse %>% 
  group_by(sector) %>% 
  count(sort = TRUE) %>% 
  arrange(desc(n)) %>% 
  rename(companies_number = n)

print(companies_number_by_sector)

ggplot(companies_number_by_sector,aes(x = reorder(sector, -companies_number), y = companies_number, colour = sector))+
...
monthly_data <- myStocks_returns_monthly %>% 
  group_by(symbol) %>% 
  summarise(min = min(monthly_returns), max = max(monthly_returns), 
...
# YOUR CODE GOES HERE
ggplot(myStocks_returns_monthly, aes(x = date, y = monthly_returns, color = symbol))+
...
> According to the plot and standard deviation for each stock, Tesla is the riskiest one. As we all know, it is a rapid growth company and one of the most significant characters of this kind of companies is taking risk. So it is a profitable but risky investment choice. And the least ricky one is SPY. It is a well-diversified basket of assets and allocate its fund into multiple factors, so it's no surprise that it is the least ricky one.

Finally, make a plot that shows the expected monthly return (mean) of a stock on the Y axis and the risk (standard deviation) in the X-axis. Please use `ggrepel::geom_text_repel()` to label each stock

```{r risk_return_plot}
ggplot(monthly_data, aes(x = SD, y = mean, colour = symbol, label = symbol))+
  geom_point(size = 4)+
  ggrepel::geom_text_repel(stat = 'identity', point.padding = 0.3,segment.size = 1)

```

What can you infer from this plot? Are there any stocks which, while being riskier, do not have a higher expected return?

> ANF is the second riskiest stock following Tesla, but its expected return is the lowest.
...
hr_cleaned %>% 
  group_by(attrition) %>% 
  count(sort = TRUE) %>% 
  ggplot(aes(x = "", y = n, fill = attrition, colour = attrition, label = n))+
  geom_bar(stat = 'identity')+
  coord_polar(theta = "y") + 
  ggrepel::geom_text_repel(colour = "black")

attrition_proportion <- 237/(237+1233)
paste("the attrition proportion is",attrition_proportion*100,"%")

```
> The proportion of attrition is around 16%.
...
```
>The age data looks more normal. Others apparently skew to right.
